...
public class PasswordUtils {
	...
	/**
	 * Generates a unique user hash
	 * 
	 * @param user The user
	 * @return The MD5 hash
	 */
	public static String userHash(final User user) {
		return StringUtils.md5(user.getId() + user.getEmail() + user.getUsername() + user.getPassword() + (user.getLastLogin() == null ? "0" : user.getLastLogin().getTime()));
	}
